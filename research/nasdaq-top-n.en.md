...
What the curve says is that there is a floor as well as a ceiling. One or two companies is not a strategy, it is a bet on a company — the edge is wild when it lands, and the share of winning windows sits near a coin flip. Past roughly a dozen names the edge decays toward zero, which it must: each name you add moves your portfolio closer to the index you are measuring against, so the gap has nowhere to go but down. The interesting region is the low-to-mid single digits.
